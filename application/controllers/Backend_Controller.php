       ...
       
{
    ...
    if ($this->form_validation->run() == FALSE) {
        ...
        $resident_data = [
            ...
            'image' => '', // Initialize the 'image' field as an empty string
        ];
    ...
    public function ajax_view_resident_form()
{
    ...
    $this->load->model('Users_model');
    $residentDetails = $this->Users_model->getResidentDetailsById($residentId);

    // Pass the resident details to the view_resident_modal.php view file
    $data['resident'] = $residentDetails;

    // Load the view_resident_modal.php view file inside the modal
    $this->load->view('backend/page/popup/view_resident_modal', $data);
}
}
   ?>